...
open class JKSubtitleView: UIView {
    ...
    private let iconSize:CGFloat = 0
    ...
    override init (frame : CGRect) {
        ...
        textLabel = UILabel(frame: CGRect(x: textLabelX, y: 0, width: frame.width - textLabelX - 2 * horizontalSpace, height: titleLableHeight))
        textLabel.isUserInteractionEnabled = true
        textLabel.textColor = UIColor.white
        textLabel.textAlignment = .center
        textLabel.font = UIFont(name: "Montserrat-Regular", size: 16)

        let subtitleLableY = textLabel.frame.origin.y + titleLableHeight
        subtitleLabel = UILabel(frame: CGRect(x: textLabelX, y: subtitleLableY, width: frame.width - textLabelX - 2 * horizontalSpace, height: subtitleLabelHeight))
    ...
    open func setTitle(_ text:String?) {
        ...
        textLabel.center.x = self.center.x
    ...
    open func setMessage(_ text:String?) {
        ...
        if subHeight <= 16.5 {
            subtitleLabel.center.x = self.center.x
        }
        baseView.frame = self.frame
    }
    ...
    open func setPanelStatus(_ status:JKStatus) {
        ...
        case .warning:
            ...
            baseView.backgroundColor = UIColor(red: 246.0/255.0, green: 185.0/255.0, blue: 20.0/255.0, alpha: 1)
        case .failed:
            imageIcon.image = loadImageBundle(named: "fail-icon")
            textLabel.text = "Failed"
            baseView.backgroundColor = UIColor(red: 188.0/255.0, green: 26.0/255.0, blue: 42.0/255.0, alpha: 1)
        }
    }
